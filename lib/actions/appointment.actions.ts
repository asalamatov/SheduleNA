...
export const getAppointment = async (appointmentId: string) => {
    try {
        ...
        return parseStringify(appointment.documents[appointment.documents.length - 1]);
    } catch (error) {
...
export const updateAppointment = async ({
    ...
    try {
        ...
        const smsMessage = `Hi, it's SheduleNA. ${
          type == 'schedule'
            ? `Your Appointment has been scheduled for ${
                formatDateTime(appointment.schedule).dateTime
              } with Mr./Ms. ${appointment.primaryAdmissionsOfficer}`
